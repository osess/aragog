...
class WubaGeneralSpider(DjangoSpider):
    # 58 新huangye
    name = 'wuba_general_spider_20160126'
    ...
    def parse_index_page(self, response):
        ...
        ul = response.css('#leftMenu')
        ul = response.xpath('//*[@id="leftMenu"]')
        logging.debug(ul)
        f = open('wuba.html', 'w')
        ...
        print 'ul'
        for a_tag in ul.xpath('li/a'):
            a_name = get_first(a_tag.xpath('text()'))
            a_href = get_first(a_tag.xpath('@href'))
            a_url = urlparse.urljoin(response.url, a_href)
            if a_name not in [u'招商加盟', u'医疗']:
                company = copy.deepcopy(meta_company)
                company['first_category'] = a_name
                yield scrapy.Request(a_url, meta={'company': company}, callback=self.parse_second_category)
    ...
    def parse_second_category(self, response):
        ...
        # todo
        if meta_company['first_category'] not in [u'商务服务', u'娱乐', u'丽人', u'美食', u'农林', u'批发', u'医疗']:

            ul = response.xpath("//ul[@class='sidebarlist']")
            for a_tag in ul.xpath('li/a'):
                a_name = get_first(a_tag.xpath('text()'))
                a_href = get_first(a_tag.xpath('@href'))
                a_url = a_href
                ...
                yield scrapy.Request(a_url, meta={'company': company}, callback=self.parse_company_list)
            ...
            for h2_tag in all_h2:
                ...
                next_url = a_href
                temp_company = copy.deepcopy(meta_company)
                temp_company['second_category'] = second_category
                if temp_company['first_category'] == u'农林':
    ...
    def parse_addr_company_detail(self, response):
        ...
        # 旅游 周边游
        if 'third_category' not in company or company['third_category'] is None or company['third_category'] == '':
            cates = response.xpath('//div[@class="cona"]').extract()
            company['third_category'] = ','.join(cates)
